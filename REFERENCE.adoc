...
inserting the `final` keyword in front of a field declaration. Or, perhaps
deleting some modifier. Such modifications are rare and should be programmed
in the generators with great care. The generators really should know what they do
as they play with a part of the code that is manually maintained and not
separated. Such modifications usually focus on some specific part
of the code. To find the specific part of the code, for example the declaration
of a given field is very hard to so reading the source as text. The list of
the lexical elements that is the result of the lexical analysis of the code
is much better suited for the purpose.

Lexical Modifications work with the list of the lexical elements of the source
files. A typical modification searches a certain part looking for some
specific pattern and then insert or replace the found pattern with a given list
of lexical elements.
...
writing. To mimic a Java record a class should have only `final` fields,
the class has to be `final`, should not extend any other class (except
the implicit Object) and should have a constructor that initializes the
`final` fields and has getters for these fields.

To do this the programmer can insert the fields into the class, but they
can not be `final` when the programmer edits the file, because the
constructor does not know about this field yet. The constructor will be
modified only later to initialize the new field or fields. After the
code generation the manual code can be altered so that the field becomes
`final`.
...
this replaces the "```class``` (spaces) class name" lexeme section in
the list with the specified "```final class``` __class__ __name__"
sequence. Note that the spaces are important in the replace part.
