 ...
 * the code only reading it and it does not touch any code. Such a generator is an auxiliary generator collecting
 ...
 *
 * <p><b>Note:</b> touching means that the generator writes some code. The actual writing may be aborted when the
 * written code is the same as the existing one but still the generator tried to do its work. If none of the generators
 * try to write (touch) any source code then what is the point to execute them? This is usually a signal of
 * misconfiguration and thus throws an exception.)
 *
