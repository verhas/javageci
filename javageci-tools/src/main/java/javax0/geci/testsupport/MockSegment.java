...
public class MockSegment implements Segment {
    ...
    public MockSegment(int tabStop) {
    ...
    public MockSegment(int tabStop, CompoundParams cparams, List<String> originals) {
        this.openingTabStop = tabStop;
        this.tabStop = tabStop;
    ...
    @Override
    public MockSegment param(String... keyValuePairs) {
    ...
    @Override
    public MockSegment write(javax0.geci.api.Segment segment) {
        if (segment != null) {
            if (segment instanceof MockSegment) {
                var other = (MockSegment) segment;
                other.lines.forEach(line -> write(line));
            } else {
                throw new GeciException("Segment " + segment + " is not instance of " + MockSegment.class.getName() +
    ...
    @Override
    public MockSegment write(String s, Object... parameters) {
        if (s != null) {
            if (!s.contains("\n") && s.trim().length() == 0) {
                newline();
            } else {
    ...
    @Override
    public MockSegment newline() {
        lines.add("");
        return this;
    }

    @Override
    public MockSegment write_r(String s, Object... parameters) {
        write(s, parameters);
        tabStop += TAB;
        return this;
    }
    ...
    @Override
    public MockSegment write_l(String s, Object... parameters) {
        tabStop -= TAB;
