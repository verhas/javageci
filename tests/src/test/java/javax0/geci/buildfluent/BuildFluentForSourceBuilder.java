...
public class BuildFluentForSourceBuilder {
    ...
    public void testSourceBuilderGeneratedApiIsGood() throws Exception{
        if (new Geci().source("./src/main/java", "./tools/src/main/java").register(new Fluent()).generate()) {
            Assertions.fail("Fluent modified source code. Please compile again.");
        }
    }

    public static FluentBuilder sourceBuilderGrammar(){
        var source = FluentBuilder.from(JavaSourceBuilder.class).start("source").cloner("copy");
        ...
        return source.zeroOrMore(source.oneOf(statement,ifStatements,whileStatement,forStatement,methodDeclaration));
    }
